...
"""
    ...
    def _cleanUpDatabase(self):
        ...
        if sqlReturn[0] == None:
            ...
            for rowOfNewStartNumbers in startNumbersOfNewIntervals:
                # find end numbers
                self._db_cur.execute("SELECT DISTINCT MIN(StartNumber)-1 FROM primeSearches WHERE StartNumber > %s" % (
                rowOfNewStartNumbers[0]))

                rowWithEndNumbers = self._db_cur.fetchone()

                self._db_cur.execute("INSERT INTO primeSearches (StartNumber,EndNumber) VALUES (%s, %s)" % (
                rowOfNewStartNumbers[0], rowWithEndNumbers[0]))
    ...
    def returnSearchInterval(self, intervalSize=10000, forceUseOfOldIntervals=False):
        ...
        if self._db_cur.rowcount > 0:
            ...
            self._db_cur.execute(
                "UPDATE primeSearches SET Startdate = CURRENT_TIMESTAMP() WHERE StartNumber = %s AND EndNumber = %s" % (
            ...
            self._db_cur.execute(
                "SELECT max(prime) from primes WHERE prime < %s AND prime > %s" % (self.endNumber, self.startNumber))
            sqlReturn = self._db_cur.fetchone()

            if not sqlReturn[0] == None:

                self._db_cur.execute(
                    "UPDATE primeSearches SET Completed = 1, EndNumber=%s WHERE StartNumber = %s AND EndNumber = %s AND Completed = 0" % (
                    sqlReturn[0], self.startNumber, self.endNumber))
                self._db_connection.commit()
                # lav nyt interval
    ...
    def returnSearchedInterval(self, StartNumber, EndNumber):
        ...
        self._db_cur.execute("UPDATE primeSearches SET Completed = 1 WHERE StartNumber = %s AND EndNumber = %s" % (
