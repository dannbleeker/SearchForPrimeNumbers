...
def main():
    ...
    if (len(sys.argv) > 1):
        if (sys.argv[1].isnumeric() and int(sys.argv[1]) > 0):  # and sys.argv[1] >0)
            ...
            print(
                "Biggest prime found: %s" % (f"{PrimeServerConnection.returnHighestPrimeFound():,d}".replace(",", ".")))
    ...
    (startSearchAt, endSearchAt) = PrimeServerConnection.returnSearchInterval(int(searchInterval),
                                                                              forceUseOfIncompleteIntervals)
